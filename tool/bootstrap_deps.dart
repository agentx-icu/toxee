...
import 'bootstrap_patch_integrity.dart';
...
void main(List<String> args) async {
  ...
  const submodulePaths = [
    'third_party/tim2tox',
    'third_party/chat-uikit-flutter',
  ];
  const submoduleUrls = {
    'third_party/tim2tox': 'https://github.com/agentx-icu/tim2tox.git',
    'third_party/chat-uikit-flutter':
        'https://github.com/agentx-icu/chat-uikit-flutter.git',
  };
  ...
  if (code != 0) {
    stdout.writeln(
      'Submodules not yet registered in repo; will clone from .gitmodules if missing.',
    );
  }
  ...
  if (uikitDir.existsSync()) {
    ...
    if (c == 0) {
      ...
      if (c == 0) {
        stdout.writeln(
          'third_party/chat-uikit-flutter switched to branch $branch',
        );
      }
    }
  }
  ...
  final lockFile = File(
    '$repoRoot/third_party/tim2tox/tool/tencent_cloud_chat_sdk.lock.json',
  );
  if (!lockFile.existsSync()) {
    stderr.writeln(
      'bootstrap_deps: missing third_party/tim2tox/tool/tencent_cloud_chat_sdk.lock.json',
    );
    stderr.writeln(
      '  Ensure third_party/tim2tox is present (clone or use tool/verify_bootstrap_local.sh for local workspace).',
    );
    exit(1);
  }
  ...
  if (version.isEmpty || archiveUrl.isEmpty) {
    stderr.writeln(
      'bootstrap_deps: lock file must have version and archive_url',
    );
    exit(1);
  }
  if (expectedSha256 == null || expectedSha256.isEmpty) {
    stderr.writeln(
      'bootstrap_deps: lock file `${lockFile.path}` missing required `sha256` field. '
  ...
  if (seriesFile.existsSync()) {
    patchNames = readPatchNames(seriesFile);
  }
  final expectedSeriesError = expectedPatchSeriesIntegrityError(
    seriesFile,
    patchNames,
    storedPatchesSha256,
    diagnosticPrefix: 'bootstrap_deps',
  );
  if (expectedSeriesError != null) {
    stderr.writeln(expectedSeriesError);
    exit(1);
  }
  if (tim2toxDir.existsSync() && seriesFile.existsSync()) {
    ...
    if (missingPatch != null) {
      stderr.writeln(
        'bootstrap_deps: patch series declares missing patch: $missingPatch',
      );
      exit(1);
    }
    patchesSha256 = computePatchesSha256(seriesFile, patchNames, patchesDir);
  }
  final patchesChanged =
      patchesSha256 != null &&
  ...
  if (needVendor) {
    ...
    try {
      ...
      final actualSha256 = await sha256File(archivePath);
      if (actualSha256 != expectedSha256) {
        throw Exception(
          'bootstrap_deps: SHA-256 mismatch (got $actualSha256, expected $expectedSha256)',
        );
      }
  ...
  final bool shouldApplySdkPatches =
  ...
  if (shouldApplySdkPatches) {
    if (patchNames.isNotEmpty) {
      stdout.writeln(
        'Applying tencent_cloud_chat_sdk patches from tim2tox (${patchNames.length} patch(es))...',
      );
      ...
      if (!scriptPath.existsSync()) {
        stderr.writeln(
          'bootstrap_deps: tim2tox tool/apply_sdk_patches.dart not found',
        );
        exit(1);
      }
      final patchCode = await _run(repoRoot, 'dart', [
        scriptPath.path,
        '--sdk-dir=${sdkDir.path}',
      ], workingDirectory: tim2toxDir.path);
      if (patchCode != 0) {
        ...
        stderr.writeln(
          'bootstrap_deps: tim2tox apply_sdk_patches failed (exit $patchCode)',
        );
        exit(patchCode);
      }
      stateMap[appliedKey] = true;
  ...
  final overrides = StringBuffer(
    '# Generated by tool/bootstrap_deps.dart - do not edit.\n',
  );
...
int _offlineCheck(String repoRoot) {
  ...
  if (!tim2tox.existsSync() || !uikit.existsSync()) {
    stderr.writeln(
      'bootstrap_deps: offline-check: submodule directory missing',
    );
    return 1;
  }
  ...
  if (!stateFile.existsSync()) {
    stderr.writeln(
      'bootstrap_deps: offline-check: tool/vendor_state.json missing',
    );
    return 1;
  }
  ...
  try {
    ...
    stderr.writeln(
      'bootstrap_deps: offline-check: failed to parse lock/state json: $e',
    );
    return 1;
  }
  ...
  if (lockVersion.isEmpty || lockSha.isEmpty) {
    stderr.writeln(
      'bootstrap_deps: offline-check: lock file missing version or sha256',
    );
    return 1;
  }
  if (state['version']?.toString() != lockVersion) {
    stderr.writeln(
      'bootstrap_deps: offline-check: vendor_state version mismatch '
  ...
  if (state['sha256']?.toString() != lockSha) {
    stderr.writeln(
      'bootstrap_deps: offline-check: vendor_state sha256 does not match lock',
    );
    return 1;
  }
  if (state['partial'] == true) {
    stderr.writeln(
      'bootstrap_deps: offline-check: vendor_state is marked partial '
  ...
  final patchIntegrityError = offlinePatchIntegrityError(
    tim2tox,
    lockVersion,
    state['patches_sha256']?.toString() ?? '',
  );
  if (patchIntegrityError != null) {
    stderr.writeln(patchIntegrityError);
    return 1;
  }
  return 0;
}
...
Future<void> _extractTarGz(String archivePath, String destDir) async {
  final code = await Process.run('tar', [
    '-xzf',
    archivePath,
    '-C',
    destDir,
  ], runInShell: false).then((r) => r.exitCode);
