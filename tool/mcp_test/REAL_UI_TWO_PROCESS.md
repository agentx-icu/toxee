...
| `group_member_info_profile_entry_opens_profile` | `group_member_info_profile_entry` | (contract changed 2026-09-17, MM-6) the peer's NGC member row carries a PER-GROUP key, not a Tox ID: the member-info route mounts (`group_member_info_copy_id_button`), labels the id as a member key (`group_member_info_key_hint` mounts) and does NOT offer the profile entry (it used to open a stranger profile / add-friend flow for a key that is nobody's Tox ID). Self and legacy-conference friends still resolve and keep the entry — L1 `test/ui/group/group_member_identity_test.dart` |
