...
class FlutterSkillBinding {
  ...
  static Future<String?> _takeScreenshot(
    ...
    try {
      ...
      final captured = await _captureFullScene(pixelRatio: pixelRatio);
      if (captured == null) {
        _log('Screenshot capture failed');
        return null;
      }
      var image = captured; // non-nullable: keeps `image` usable in finally

      // toxee vendored fix: every image/picture below is released via
      // try/finally — 0.9.36 leaked the physical-size capture (~16MB on a
      // 2x display) on every shot, so long test runs killed the app; and a
      // throwing toImage/toByteData must not leak either.
      try {
  ...
  static Future<String?> _takeRegionScreenshot(
    ...
    try {
      ...
      // toxee vendored fix: dispose capture/picture/crop via try/finally so
      // no failure path can leak them.
      try {
  ...
  static Future<String?> _takeElementScreenshot(String key) async {
    try {
      ...
      if (renderObject is RenderRepaintBoundary) {
        ...
        // toxee vendored fix: release the capture even if encoding throws.
        try {
          final byteData =
      ...
      while (current != null) {
        if (current is RenderRepaintBoundary) {
          ...
          if (renderObject is RenderBox) {
            ...
            // toxee vendored fix: dispose capture/picture/crop via
            // try/finally so no failure path can leak them.
            try {
