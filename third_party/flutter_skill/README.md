> **toxee vendored copy** of pub.dev `flutter_skill` 0.9.36 (MIT), library-only
> (`lib/` + metadata; no `bin/`, `example/`, `test/`). Local change: every
> screenshot path disposes its `ui.Image`/`ui.Picture` via try/finally —
> upstream 0.9.36 leaks the physical-size capture on every shot. Drop this
> vendor when upstream ships the fix (see `tool/bootstrap_deps.dart`).
